...
#define DWT_A0_DEV_ID       (0xDECA0300)        //!< DWM3000 MPW A0 (non PDOA) silicon device ID
#define DWT_A0_PDOA_DEV_ID  (0xDECA0310)        //!< DWM3000 MPW A0 (with PDOA) silicon device ID
#define DWT_B0_DEV_ID       (0xDECA0301)        //!< DWM3000 MPW B0 (non PDOA) silicon device ID
#define DWT_B0_PDOA_DEV_ID  (0xDECA0311)        //!< DWM3000 MPW B0 (with PDOA) silicon device ID
#define DWT_C0_DEV_ID       (0xDECA0302)        //!< DWM3000 MPW C0 (non PDOA) silicon device ID
#define DWT_C0_PDOA_DEV_ID  (0xDECA0312)        //!< DWM3000 MPW C0 (with PDOA) silicon device ID
...
typedef enum {
    DWM3000_SPI_RD_BIT    = 0x0000U,
    DWM3000_SPI_WR_BIT    = 0x8000U,
    DWM3000_SPI_AND_OR_8  = 0x8001U,
    DWM3000_SPI_AND_OR_16 = 0x8002U,
    DWM3000_SPI_AND_OR_32 = 0x8003U,
}spi_modes_e;
...
#define CMD_TXRXOFF       0x0    //!< Turn off TX or RX, clear any TX/RX events and put DWM3000 into IDLE
...
typedef enum
{
    ...
    DWT_SPI_CRC_MODE_WR,        /* This is used to enable SPI CRC check (the SPI CRC check will be enabled on DWM3000 and CRC-8 added for SPI write transactions) */
...
//DWM3000 SLEEP and WAKEUP configuration parameters
...
//DWM3000 IDLE/INIT mode definitions
...
//DWM3000 OTP operating parameter set selection
...
// Call-back type for SPI read error event (if the DWM3000 generated CRC does not match the one calculated by the dwt_generatecrc8 function)
...
typedef struct
{
    //all of the below are mapped to a register in DWM3000
...
//enums below are defined in such a way as to allow a simple write to DWM3000 AES configuration registers
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief This function returns the version of the API as defined by DWM3000_DRIVER_VERSION
 *
 * input parameters
 *
 * output parameters
 *
 * returns version (DWM3000_DRIVER_VERSION)
 */
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief This is used to return the read device type and revision information of the DWM3000 device (MP part is 0xDECA0130)
 *
 * input parameters
 *
 * output parameters
 *
 * returns the read value which for DWM3000 is 0xDECA0130
 */
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief This is used to enable GPIO for external LNA or PA functionality - HW dependent, consult the DWM3000 User Manual.
 *        This can also be used for debug as enabling TX and RX GPIOs is quite handy to monitor DWM3000's activity.
 *
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief This function initialises the DWM3000 transceiver:
 * it reads its DEV_ID register (address 0x00) to verify the IC is one supported
 * by this software (e.g. DWM3000 32-bit device ID value is 0xDECA03xx).  Then it
 ...
 * 3.it is assumed this function is called after a reset or on power up of the DWM3000*
 *
 * input parameters
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief This function can place DWM3000 into IDLE/IDLE_PLL or IDLE_RC mode when it is not actively in TX or RX.
 *
 * input parameters
 * @param state - DWT_DW_IDLE (1) to put DWM3000 into IDLE/IDLE_PLL state; DWT_DW_INIT (0) to put DWM3000 into INIT_RC state;
 *                DWT_DE_IDLE_RC (2) to put DWM3000 into IDLE_RC state.
 *
 * output parameters none
 *
 * no return value
 ...
 * DWM3000 and this low-level driver.  The input is a pointer to the data structure
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief This function configures the STS AES 128 bit initial value, the default value is 1, i.e. DWM3000 reset value is 1.
 *
 * input parameters
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief This API function writes the supplied TX data into the DWM3000's
 ...
 * See DWM3000 User Manual section 4.5 "Low-Power SNIFF mode" for more details.
 *
 * input parameters:
...
/*! ------------------------------------------------------------------------------------------------------------------
*
* @brief this function clears the AON configuration in DWM3000
*
* input parameters:
*
* output parameters
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief This function checks if the DWM3000 is in IDLE_RC state
 *
 * input parameters
 *
 * output parameters
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief This is the DWM3000's general Interrupt Service Routine. It will process/report the following events:
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief This is used to get the EUI 64-bit from the DWM3000
 *
 * input parameters
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief This is used to enable SPI CRC check in DWM3000
 *
 * input parameters
 * @param crc_mode - if set to DWT_SPI_CRC_MODE_WR then SPI CRC checking will be performed in DWM3000 on each SPI write
 *                   last byte of the SPI write transaction needs to be the 8-bit CRC, if it does not match
 *                   the one calculated by DWM3000 SPI CRC ERROR event will be set in the status register (SYS_STATUS_SPICRC)
 *
 * @param spireaderr_cb - this needs to contain the callback function pointer which will be called when SPI read error
 *                        is detected (when the DWM3000 generated CRC does not match the one calculated by  dwt_generatecrc8
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief this function resets the DWM3000
 *
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief This is used to read the crystal offset (relating to the frequency offset of the far DWM3000 device compared to this one)
 ...
 *
 * input parameters
 * @param diagnostics - diagnostic structure pointer, this will contain the diagnostic data read from the DWM3000
 *
 * output parameters
 *
 * no return value
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief This is used to program 32-bit value into the DWM3000 OTP memory.
 *
 * input parameters
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief this function sets the DWM3000 to transmit cw signal at specific channel frequency
 *
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief this function sets the DWM3000 to continuous tx frame mode for regulatory approvals testing.
 *
 * input parameters:
 ...
 * ensure pdwm3000local->tempP contains the SAR_LTEMP value from OTP.
 *
 * input parameters:
 ...
 * ensure pdwm3000local->vBatP contains the SAR_LVBAT value from OTP
 *
 * input parameters:
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief this function reads the temperature of the DWM3000 that was sampled
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief this function reads the battery voltage of the DWM3000 that was sampled
 * on waking from Sleep/Deepsleep. They are not current values, but read on last
 * wakeup if DWT_TANDV bit is set in mode parameter of dwt_configuresleep
 *
 * input parameters:
 ...
 * of the DWM3000. The adjustemnt is a result of DWM3000 internal PG cal routine, given a target count value it will try to
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief  this function is used to write to the DWM3000 device registers
 * Notes:
 ...
 *
 *        2. Write the header followed by the data bytes to the DWM3000 device
 *
 *
 * input parameters:
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief  this function is used to read from the DWM3000 device registers
 * Notes:
 *        1. Firstly we create a header (the first byte is a header byte)
 *        a. check if sub index is used, if subindexing is used - set bit-6 to 1 to signify that the sub-index address follows the register index byte
 *        b. set bit-7 (or with 0x80) for write operation
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief  this function is used to read 32-bit value from the DWM3000 device registers
 *
 * input parameters:
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief  this function is used to write 32-bit value to the DWM3000 device registers
 *
 * input parameters:
 * @param regFileID - ID of register file or buffer being accessed
 * @param regOffset - the index into register file or buffer being accessed
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief  this function is used to read 16-bit value from the DWM3000 device registers
 *
 * input parameters:
 * @param regFileID - ID of register file or buffer being accessed
 * @param regOffset - the index into register file or buffer being accessed
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief  this function is used to write 16-bit value to the DWM3000 device registers
 *
 * input parameters:
 * @param regFileID - ID of register file or buffer being accessed
 * @param regOffset - the index into register file or buffer being accessed
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief  this function is used to read an 8-bit value from the DWM3000 device registers
 *
 * input parameters:
 * @param regFileID - ID of register file or buffer being accessed
 * @param regOffset - the index into register file or buffer being accessed
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief  this function is used to write an 8-bit value to the DWM3000 device registers
 *
 * input parameters:
 * @param regFileID - ID of register file or buffer being accessed
 * @param regOffset - the index into register file or buffer being accessed
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief  this function can be used to modify a 32-bit register of DWM3000
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief  this function can be used to modify a 16-bit register of DWM3000
 *         Used to optimize SPI transaction when only clear or set of know
 *         bits in the register is requierd
 *
 * input parameters:
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief  this function can be used to modify a 8-bit register of DWM3000
 *         Used to optimize SPI transaction when only clear or set of know
 *         bits in the register is requierd
 *
 * input parameters:
 ...
 * Low level abstract function to write to the SPI, this should be used when DWM3000 SPI CRC mode is used
 ...
 * output parameters
 *
 * returns the state of the DWM3000 interrupt
 */
 ...
 *
 * input parameters:
 * @param s - the state of the DWM3000 interrupt as returned by decamutexon
 *
 * output parameters
 *
 * returns the state of the DWM3000 interrupt
