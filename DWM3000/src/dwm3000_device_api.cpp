...
//
#define DWM3000_SPI_FAC      (0<<6 | 1<<0)
#define DWM3000_SPI_FARW     (0<<6 | 0<<0)
#define DWM3000_SPI_EAMRW    (1<<6)
...
// -------------------------------------------------------------------------------------------------------------------
// Data for DWM3000 Decawave Transceiver control
//
...
static localdata *pdwm3000local = &_ptr;
...
{
    return DWM3000_DRIVER_VERSION;
}

/*! ------------------------------------------------------------------------------------------------------------------
* @brief  this function is used to read/write to the DWM3000 device registers
*
* input parameters:
...
* @param rw            - DWM3000_SPI_WR_BIT/DWM3000_SPI_RD_BIT
*
* no return value
*/
static
...
)
{
    ...
    assert(mode == DWM3000_SPI_WR_BIT ||\
           mode == DWM3000_SPI_RD_BIT ||\
           mode == DWM3000_SPI_AND_OR_8 ||\
           mode == DWM3000_SPI_AND_OR_16 ||\
           mode == DWM3000_SPI_AND_OR_32);
    ...
    {   /* Fast Access Commands (FAC)
        ...
        header[0] = (uint8_t)((DWM3000_SPI_WR_BIT>>8) | (regFileID<<1) | DWM3000_SPI_FAC);
        cnt = 1;
    }
    else if (reg_offset == 0 /*&& length > 0*/ && (mode == DWM3000_SPI_WR_BIT || mode == DWM3000_SPI_RD_BIT))
    ...
    {   /* Extended Address Mode with Read/Write support (EAMRW)
         ...
         * b[1] = addr<<2 | (mode&0x3)
         */
        header[0] |= DWM3000_SPI_EAMRW;
        cnt = 2;
    }

    switch (mode)
    {
    case    DWM3000_SPI_AND_OR_8:
    case    DWM3000_SPI_AND_OR_16:
    case    DWM3000_SPI_AND_OR_32:
    case    DWM3000_SPI_WR_BIT:
    {
    {
...
{
    dwt_xfer3000(regFileID, index, length, buffer, DWM3000_SPI_WR_BIT);
}

/*! ------------------------------------------------------------------------------------------------------------------
 * @brief  This function wakeup device by an IO pin. DWM3000 SPI_CS or WAKEUP pins can be used for this.
 *         wakeup_device_with_io() which is external to this file and is platform dependant and it should be modified to
 *         toggle the correct pin depending on the HW/MCU connections with DWM3000.
 *
 * @param None
 *
 * output parameters
...
{
    dwt_xfer3000(regFileID, index, length, buffer, DWM3000_SPI_RD_BIT);
}
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief  this function is used to modify a 32-bit value to the DWM3000 device registers
 *
 * input parameters:
 * @param regFileID :   ID of register file or buffer being accessed
 * @param regOffset :   the index into register file or buffer being accessed
...
void dwt_modify32bitoffsetreg(const uint32_t regFileID, const uint16_t regOffset, const uint32_t _and, const uint32_t _or)
{
    ...
    dwt_xfer3000(regFileID, regOffset, sizeof(buf), buf, DWM3000_SPI_AND_OR_32);
}

/*! ------------------------------------------------------------------------------------------------------------------
 * @brief  this function is used to modify a 16-bit value to the DWM3000 device registers
 *
 * input parameters:
 * @param regFileID :   ID of register file or buffer being accessed
 * @param regOffset :   the index into register file or buffer being accessed
...
void dwt_modify16bitoffsetreg(const uint32_t regFileID, const uint16_t regOffset, const uint16_t _and, const uint16_t _or)
{
    ...
    dwt_xfer3000(regFileID, regOffset, sizeof(buf), buf, DWM3000_SPI_AND_OR_16);
}

/*! ------------------------------------------------------------------------------------------------------------------
 * @brief  this function is used to modify a 8-bit value to the DWM3000 device registers
 *
 * input parameters:
 * @param regFileID :   ID of register file or buffer being accessed
 * @param regOffset :   the index into register file or buffer being accessed
...
void dwt_modify8bitoffsetreg(const uint32_t regFileID, const uint16_t regOffset, const uint8_t _and, const uint8_t _or)
{
    ...
    buf[0] = _and;
    buf[1] = _or;
    dwt_xfer3000(regFileID, regOffset, sizeof(buf),buf, DWM3000_SPI_AND_OR_8);
}
...
{
    if (crc_mode != DWT_SPI_CRC_MODE_NO) //enable CRC check in DWM3000
    {
        dwt_or8bitoffsetreg(SYS_CFG_ID, 0, SYS_CFG_SPI_CRC_BIT_MASK);

        if (crc_mode == DWT_SPI_CRC_MODE_WRRD) //enable CRC generation on SPI read transaction which the DWM3000 will store in SPICRC_CFG_ID register
        {
            pdwm3000local->cbSPIRDErr = spireaderr_cb;
        }
...
void _dwt_prog_ldo_and_bias_tune(void)
{
    ...
    dwt_and_or16bitoffsetreg(BIAS_CTRL_ID, 0, (uint16_t)~BIAS_CTRL_BIAS_MASK, pdwm3000local->bias_tune);
}
...
void _dwt_kick_ops_table_on_wakeup(void)
{
    ...
    switch (pdwm3000local->sleep_mode & (DWT_ALT_OPS | DWT_SEL_OPS0 | DWT_SEL_OPS1 | DWT_SEL_OPS2 | DWT_SEL_OPS3))
    {
 ...
 * 2.it is assumed this function is called after a reset or on power up of the DWM3000
 *
 * input parameters
...
int dwt_initialise(uint8_t mode)
{
    ...
    pdwm3000local->dblbuffon = DBL_BUFF_OFF; // Double buffer mode off by default / clear the flag
    pdwm3000local->sleep_mode = DWT_RUNSAR;  // Configure RUN_SAR on wake by default as it is needed when running PGF_CAL
    pdwm3000local->spicrc = DWT_SPI_CRC_MODE_NO;
    pdwm3000local->stsconfig = 0; //STS off
    pdwm3000local->vBatP = 0;
    pdwm3000local->tempP = 0;

    pdwm3000local->cbTxDone = NULL;
    pdwm3000local->cbRxOk = NULL;
    pdwm3000local->cbRxTo = NULL;
    pdwm3000local->cbRxErr = NULL;
    pdwm3000local->cbSPIRdy = NULL;
    pdwm3000local->cbSPIErr = NULL;
    ...
    pdwm3000local->bias_tune = (_dwt_otpread(BIAS_TUNE_ADDRESS) >> 16) & BIAS_CTRL_BIAS_MASK;

    if ((ldo_tune_lo != 0) && (ldo_tune_hi != 0) && (pdwm3000local->bias_tune != 0))
    {
    ...
    {
        pdwm3000local->dgc_otp_set = DWT_DGC_LOAD_FROM_OTP;
    }
    else
    {
        pdwm3000local->dgc_otp_set = DWT_DGC_LOAD_FROM_SW;
    }

    // Load Part and Lot ID from OTP
    if(!(mode & DWT_READ_OTP_PID))
  {
        pdwm3000local->partID = _dwt_otpread(PARTID_ADDRESS);
  }
    if (!(mode & DWT_READ_OTP_LID))
  {
        pdwm3000local->lotID = _dwt_otpread(LOTID_ADDRESS);
  }
  if (!(mode & DWT_READ_OTP_BAT))
  {
        pdwm3000local->vBatP = (uint8_t)_dwt_otpread(VBAT_ADDRESS);
  }
  if (!(mode & DWT_READ_OTP_TMP))
  {
        pdwm3000local->tempP = (uint8_t)_dwt_otpread(VTEMP_ADDRESS);
  }

    if(pdwm3000local->tempP == 0) //if the reference temperature has not been programmed in OTP (early eng samples) set to default value
    {
        pdwm3000local->tempP = 0x85 ; //@temp of 20 deg
    }

    if(pdwm3000local->vBatP == 0) //if the reference voltage has not been programmed in OTP (early eng samples) set to default value
    {
        pdwm3000local->vBatP = 0x74 ;  //@Vref of 3.0V
    }

    pdwm3000local->otprev = (uint8_t) _dwt_otpread(OTPREV_ADDRESS);

    pdwm3000local->init_xtrim = _dwt_otpread(XTRIM_ADDRESS) & 0x7f;
    if(pdwm3000local->init_xtrim == 0)
    {
        pdwm3000local->init_xtrim = 0x2E ; //set default value
    printf("XTRIM OTP READ FAIL\r\n");
    }
    dwt_write8bitoffsetreg(XTAL_ID, 0, pdwm3000local->init_xtrim);
 ...
 *                DWT_DW_IDLE_RC (2) to put DWM3000 into IDLE_RC state.
 *
...
{
    if (state == DWT_DW_IDLE) // Set the auto INIT2IDLE bit so that DWM3000 enters IDLE mode before switching clocks to system_PLL
...
{
    return pdwm3000local->otprev ;
}

/*! ------------------------------------------------------------------------------------------------------------------
 * @brief This function enables/disables the fine grain TX sequencing (this is enabled by default in the DWM3000).
 *
 * input parameters
...
{
    return pdwm3000local->lotID;
}

/*! ------------------------------------------------------------------------------------------------------------------
 * @brief This is used to return the read device type and revision information of the DWM3000 device (MP part is 0xDECA0300)
 *
 * input parameters
 *
 * output parameters
 *
 * returns the read value which for DWM3000 is 0xDECA0312/0xDECA0302
 */
...
void dwt_restoreconfig(void)
{
    ...
    if( (((chan_ctrl>> CHAN_CTRL_TX_PCODE_BIT_OFFSET)&CHAN_CTRL_TX_PCODE_BIT_MASK) >= 9) && (((chan_ctrl >> CHAN_CTRL_TX_PCODE_BIT_OFFSET)&CHAN_CTRL_TX_PCODE_BIT_MASK) <= 24)) //only enable DGC for PRF 64
    {
        ...
        if (pdwm3000local->dgc_otp_set == DWT_DGC_LOAD_FROM_OTP)
        {
...
void dwt_configurestsmode(uint8_t stsMode)
{
    ...
    pdwm3000local->stsconfig = stsMode;
...
int dwt_configure(dwt_config_t *config)
{
  ...
  assert(((config->stsMode & DWT_STS_CONFIG_MASK) == DWT_STS_MODE_OFF)
    ...
    pdwm3000local->sleep_mode &= (~(DWT_ALT_OPS | DWT_SEL_OPS3));  //clear the sleep mode ALT_OPS bit
    pdwm3000local->longFrames = config->phrMode ;
    sts_len=GET_STS_REG_SET_VALUE((uint16_t)(config->stsLength));
    pdwm3000local->ststhreshold = (int16_t)((((uint32_t)sts_len) * 8) * STSQUAL_THRESH_64);
    pdwm3000local->stsconfig = config->stsMode;
    ...
    if (scp)
    {
        ...
        pdwm3000local->sleep_mode |= DWT_ALT_OPS | DWT_SEL_OPS1;  //configure correct OPS table is kicked on wakeup
        dwt_modify32bitoffsetreg(OTP_CFG_ID, 0, ~(OTP_CFG_OPS_ID_BIT_MASK), DWT_OPSET_SCP | OTP_CFG_OPS_KICK_BIT_MASK);
    ...
    else
    {
        ...
        if (preamble_len >= 256)
        {
            pdwm3000local->sleep_mode |= DWT_ALT_OPS | DWT_SEL_OPS0;
            dwt_modify32bitoffsetreg(OTP_CFG_ID, 0, ~(OTP_CFG_OPS_ID_BIT_MASK), DWT_OPSET_LONG | OTP_CFG_OPS_KICK_BIT_MASK);
        }
        else
        {
...
{
#ifdef DWT_API_ERROR_CHECK
    assert((pdwm3000local->longFrames && (txDataLength <= EXT_FRAME_LEN)) ||\
...
void dwt_writetxfctrl(uint16_t txFrameLength, uint16_t txBufferOffset, uint8_t ranging)
{
    uint32_t reg32;
#ifdef DWT_API_ERROR_CHECK
    assert((pdwm3000local->longFrames && (txFrameLength <= EXT_FRAME_LEN)) ||\
           (txFrameLength <= STD_FRAME_LEN));
#endif

    //DWM3000/3700 - if offset is > 127, 128 needs to be added before data is written, this will be subtracted internally
    ...
    else
    {
        ...
        dwt_modify32bitoffsetreg(TX_FCTRL_ID, 0, ~(TX_FCTRL_TXB_OFFSET_BIT_MASK | TX_FCTRL_TR_BIT_MASK | TX_FCTRL_TXFLEN_BIT_MASK), reg32);
        reg32 = dwt_read8bitoffsetreg(SAR_CTRL_ID, 0); //DWM3000/3700 - need to read this to load the correct TX buffer offset value
    }
...
void dwt_readrxdata(uint8_t *buffer, uint16_t length, uint16_t rxBufferOffset)
{
    ...
    if (pdwm3000local->dblbuffon == DBL_BUFF_ACCESS_BUFFER_1)  //if the flag is 0x3 we are reading from RX_BUFFER_1
    {
...
int16_t dwt_readclockoffset(void)
{
    ...
    switch (pdwm3000local->dblbuffon)  //if the flag is non zero - we are either accessing RX_BUFFER_0 or RX_BUFFER_1
    {
...
int dwt_readstsquality(int16_t* rxStsQualityIndex)
{
    ...
    if(preambleCount & STS_ACC_CP_QUAL_SIGNTST)
  {
    ...
    *rxStsQualityIndex = (int16_t)preambleCount;
    ...
    return (int)((int16_t)preambleCount - pdwm3000local->ststhreshold);
}
...
int dwt_readstsstatus(uint16_t* stsStatus, uint8_t sts_num)
{
    ...
    switch (pdwm3000local->dblbuffon) //check if in double buffer mode and if so which buffer host is currently accessing
    {
    case DBL_BUFF_ACCESS_BUFFER_1:
        //!!! Assumes that Indirect pointer register B was already set. This is done in the dwt_setdblrxbuffmode when mode is enabled.
...
void dwt_readdiagnostics(dwt_rxdiag_t *diagnostics)
{
    ...
    case DBL_BUFF_ACCESS_BUFFER_0:
        ...
        if (pdwm3000local->dblbuffon == DBL_BUFF_ACCESS_BUFFER_1)
        {
            /* Program the indirect offset registers B for specified offset to swinging set buffer B */
            //!!! Assumes that Indirect pointer register B was already set. This is done in the dwt_setdblrxbuffmode when mode is enabled.
            /* Indirectly read data from the IC to the buffer */
            if (pdwm3000local->cia_diagnostic & DW_CIA_DIAG_LOG_MAX)
            {
                dwt_readfromdevice(INDIRECT_POINTER_B_ID, 0, DB_MAX_DIAG_SIZE, temp);
            }
            else if (pdwm3000local->cia_diagnostic & DW_CIA_DIAG_LOG_MID)
            {
        ...
        if (diagnostics->pdoa & 0x2000)
    {
      diagnostics->pdoa |= 0xC000; //sign extend
        ...
        if (pdwm3000local->cia_diagnostic & DW_CIA_DIAG_LOG_MIN)
    {
            break;
    }
        ...
        if (pdwm3000local->cia_diagnostic & DW_CIA_DIAG_LOG_MID)
    {
            break;
    }
    ...
    default:  //double buffer is off
        ...
        if (pdwm3000local->cia_diagnostic & DW_CIA_DIAG_LOG_ALL)
        {
        ...
        if (diagnostics->pdoa & 0x2000)
    {
      diagnostics->pdoa |= 0xC000; //sign extend
        ...
        if ((pdwm3000local->cia_diagnostic & DW_CIA_DIAG_LOG_ALL) == 0)
    {
...
static void _dwt_otpprogword32(uint32_t data, uint16_t address)
{
  ...
  __dwt_otp_write_wdata_id_reg(0x0101);
    ...
    1000 is more than sufficient for max OTP programming delay and max supported DWM3000 SPI rate.
 ...
 *      DWT_PRESRVE_SLP  0x20 - allows for SLEEP_EN bit to be "preserved", although it will pdwm3000local-clear on wake up
...
void dwt_configuresleep(uint16_t mode, uint8_t wake)
{
    ...
    pdwm3000local->sleep_mode |= mode;
    dwt_write16bitoffsetreg(AON_DIG_CFG_ID, 0,  pdwm3000local->sleep_mode);
...
void dwt_configciadiag(uint8_t enable_mask)
{
    ...
    pdwm3000local->cia_diagnostic = enable_mask;
}
...
void dwt_signal_rx_buff_free(void)
{
    ...
    if (pdwm3000local->dblbuffon == DBL_BUFF_ACCESS_BUFFER_1)
    {
        pdwm3000local->dblbuffon = DBL_BUFF_ACCESS_BUFFER_0;  //next buffer is RX_BUFFER_0
    }
    else
    {
        pdwm3000local->dblbuffon = DBL_BUFF_ACCESS_BUFFER_1;  //next buffer is RX_BUFFER_1
    }
}
...
void dwt_setdblrxbuffmode(dwt_dbl_buff_state_e dbl_buff_state, dwt_dbl_buff_mode_e dbl_buff_mode)
{
    ...
    if (dbl_buff_state==DBL_BUF_STATE_EN)
    {
        ...
        pdwm3000local->dblbuffon = DBL_BUFF_ACCESS_BUFFER_0;    //the host will access RX_BUFFER_0 initially (on 1st reception after enable)
    ...
    else
    {
        ...
        pdwm3000local->dblbuffon = DBL_BUFF_OFF;
    }
...
{
    pdwm3000local->cbTxDone = cbTxDone;
    pdwm3000local->cbRxOk = cbRxOk;
    pdwm3000local->cbRxTo = cbRxTo;
    pdwm3000local->cbRxErr = cbRxErr;
    pdwm3000local->cbSPIErr = cbSPIErr;
    pdwm3000local->cbSPIRdy = cbSPIRdy;
}
...
void dwt_isr(void)
{
    ...
    pdwm3000local->cbData.status = status;
    if ((pdwm3000local->stsconfig & DWT_STS_MODE_ND) == DWT_STS_MODE_ND) //cannot use FSTAT when in no data mode...
    {
    ...
    {
        pdwm3000local->cbData.status_hi = dwt_read16bitoffsetreg(SYS_STATUS_HI_ID, 0);
        ...
        if((pdwm3000local->spicrc && (pdwm3000local->cbData.status & SYS_STATUS_SPICRCE_BIT_MASK)) ||
                (pdwm3000local->cbData.status_hi & (SYS_STATUS_HI_SPIERR_BIT_MASK | SYS_STATUS_HI_SPI_UNF_BIT_MASK | SYS_STATUS_HI_SPI_OVF_BIT_MASK)))
        {
            ...
            if(pdwm3000local->cbSPIErr != NULL)
            {
                pdwm3000local->cbSPIErr(&pdwm3000local->cbData);
            }
        }
        ...
        if(pdwm3000local->cbData.status_hi & SYS_STATUS_HI_CMD_ERR_BIT_MASK)
        {
            dwt_write16bitoffsetreg(SYS_STATUS_HI_ID, 0, SYS_STATUS_HI_CMD_ERR_BIT_MASK); // Clear CMD error event bit
            // Call the corresponding callback if present
            /*if(pdwm3000local->cbCMDErr != NULL)
            {
                pdwm3000local->cbCMDErr(&pdwm3000local->cbData);
            }*/
        }
    ...
    if(fstat & FINT_STAT_TXOK_BIT_MASK)
    {
        ...
        // Call the corresponding callback if present
        if(pdwm3000local->cbTxDone != NULL)
        {
            pdwm3000local->cbTxDone(&pdwm3000local->cbData);
        }
    ...
    {
        //pdwm3000local->cbData.status_hi = dwt_read16bitreg(SYS_STATUS_HI_ID);

        // Call the corresponding callback if present
        if(pdwm3000local->cbSPIRdy != NULL)
        {
            pdwm3000local->cbSPIRdy(&pdwm3000local->cbData);
        }
    ...
    if(fstat & FINT_STAT_RXOK_BIT_MASK)
    {
        ...
        pdwm3000local->cbData.rx_flags = 0;

        if(pdwm3000local->dblbuffon) // if in double buffer mode
        {
            uint8_t statusDB = dwt_read8bitoffsetreg(RDB_STATUS_ID,0);

            if (pdwm3000local->dblbuffon == DBL_BUFF_ACCESS_BUFFER_1) //If accessing the second buffer (RX_BUFFER_B then read second nibble of the DB status reg)
            {
                statusDB >>= 4;
            }
      ...
      if (statusDB & RDB_STATUS_CIADONE0_BIT_MASK)
      {
        ...
        {
            pdwm3000local->cbData.rx_flags |= DWT_CB_DATA_RX_FLAG_CER;
        ...
        else
        {
            ...
            {
                pdwm3000local->cbData.rx_flags |= DWT_CB_DATA_RX_FLAG_CIA;
            }
        }

        if (status & SYS_STATUS_CPERR_BIT_MASK)
        {
            pdwm3000local->cbData.rx_flags |= DWT_CB_DATA_RX_FLAG_CPER;
            cia_err |= SYS_STATUS_CPERR_BIT_MASK;
        }

        // When using No Data STS mode we do not get RXFCG but RXFR
        if ((status & SYS_STATUS_RXFR_BIT_MASK) && ((pdwm3000local->stsconfig & DWT_STS_MODE_ND) == DWT_STS_MODE_ND))
        {
            pdwm3000local->cbData.rx_flags |= DWT_CB_DATA_RX_FLAG_ND;
            pdwm3000local->cbData.datalength = 0;
        ...
        if (status & SYS_STATUS_RXFCG_BIT_MASK)
        {
            ...
            if(pdwm3000local->longFrames == 0)
            {
                pdwm3000local->cbData.datalength = finfo16 & RX_FINFO_STD_RXFLEN_MASK;
            }
            else
            {
                pdwm3000local->cbData.datalength = finfo16 & RX_FINFO_RXFLEN_BIT_MASK;
            }

            // Report ranging bit
            if(finfo16 & RX_FINFO_RNG_BIT_MASK)
            {
                pdwm3000local->cbData.rx_flags |= DWT_CB_DATA_RX_FLAG_RNG;
            }
        ...
        // Call the corresponding callback if present
        if(pdwm3000local->cbRxOk != NULL)
        {
            pdwm3000local->cbRxOk(&pdwm3000local->cbData);
        }

        if (pdwm3000local->dblbuffon)   //check if in double buffer mode and if so which buffer host is currently accessing
        {
    ...
    if(fstat & FINT_STAT_RXERR_BIT_MASK)
    {
        ...
        // Call the corresponding callback if present
        if(pdwm3000local->cbRxErr != NULL)
        {
            pdwm3000local->cbRxErr(&pdwm3000local->cbData);
        }
    ...
    if(fstat & FINT_STAT_RXTO_BIT_MASK)
    {
        ...
        // Call the corresponding callback if present
        if(pdwm3000local->cbRxTo != NULL)
        {
            pdwm3000local->cbRxTo(&pdwm3000local->cbData);
        }
...
void dwt_softreset(void)
{
    ...
    // DWM3000 needs a 10us sleep to let clk PLL lock after reset - the PLL will automatically lock after the reset
    ...
    pdwm3000local->dblbuffon = DBL_BUFF_ACCESS_BUFFER_0;
    pdwm3000local->sleep_mode = 0;
...
void dwt_setxtaltrim(uint8_t value)
{
    ...
    pdwm3000local->init_xtrim = reg_val;
...
{
    return pdwm3000local->init_xtrim;
}
...
float dwt_convertrawtemperature(uint8_t raw_temp)
{
    ...
    realtemp = (float)((raw_temp - pdwm3000local->tempP) * 1.05f) + 20.0f;
    return realtemp;
}
...
float dwt_convertrawvoltage(uint8_t raw_voltage)
{
    ...
    realvolt = (float)((float)(raw_voltage - pdwm3000local->vBatP) * 0.4f * 16 / 255) + 3.0f;
 ...
 * of the DWM3000. The adjustment is a result of DWM3000 internal PG cal routine, given a target count value it will try to
 ...
 *
 * DWM3000 IC stores the nonce in AES_IV0_ID to AES_IV3_ID registers.
 * DWM3000 IC, when operating in CCM* AES mode expects the nonce to be programmed into 4 words as follows:
 ...
 *
 * DWM3000 IC, when operating in GCM AES mode expects the nonce to be programmed into 3 words as follows:
