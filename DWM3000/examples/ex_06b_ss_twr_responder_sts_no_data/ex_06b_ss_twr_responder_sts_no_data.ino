...
void setup() {
  ...
  /* Configure SPI rate, DWM3000 supports up to 38 MHz */
  /* Reset DW IC */
  spiBegin(PIN_IRQ, PIN_RST);
  spiSelect(PIN_SS);

  delay(2); // Time needed for DWM3000 to start up (transition from INIT_RC to IDLE_RC, or could wait for SPIRDY event)
  ...
  /* Enabling LEDs here for debug so that for each TX the D1 LED will flash on DWM3000 red eval-shield boards.
 ...
 * 9. When running this example on the DWM3000 platform with the POLL_RX_TO_RESP_TX_DLY response delay provided, the dwt_starttx() is always
 ...
 * 10. The user is referred to DecaRanging ARM application (distributed with DWM3000 product) for additional practical example of usage, and to the
